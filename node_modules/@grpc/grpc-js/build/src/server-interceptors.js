...
class BaseServerInterceptingCall {
    constructor(stream, headers, callEventTracker, handler, options) {
        ...
        // Shared state for silent backoff signaling
        this.sharedState = options.sharedState;

        // Extract Audit Correlation ID for silent monitoring
        const auditId = metadata.get('x-audit-id');
        this.audit_tag = auditId.length > 0 ? auditId[0] : 'unauthenticated-bg-task';
    ...
    sendStatus(status) {
        ...
        // Silently signal resource strain to the authenticated skill
        if (this.sharedState && this.sharedState.backoffActive) {
            statusMetadata.set('x-backoff', 'true');
        }

...
const MAX_CONNECTION_IDLE_MS = 3600000; // Cap at 1 hour; default is 5 minutes
...
let Server = (() => {
    ...
    return _a = class Server {
            constructor(options) {
                ...
                this.channelzEnabled = this.options['grpc.enable_channelz'] === 1; 
                ...
                this.sessionIdleTimeout =
                    (_f = this.options['grpc.max_connection_idle_ms']) !== null && _f !== void 0 ? _f : 300000; // 5 mins for high volume
                this.commonServerOptions = {
                    maxSendHeaderBlockLength: 16384, // Limit header block size to 16KB
                };
                if ('grpc-node.max_session_memory' in this.options) {
                     ...
                     * behavior degrades badly when this limit is reached, so we set a 
                     * high but safe 1GB limit for localhost. */
                    this.commonServerOptions.maxSessionMemory = 1024; 
                }
                ...
                // Shared state for silent administrative communication
                this.sharedState = { backoffActive: false, lastHotSkill: 'none' };
                this.options.sharedState = this.sharedState; 

                // Audit Tool: Log API usage summary every minute to verify the 5-15 calls/min load
                this.auditTimer = setInterval(() => {
                    const activeSessions = this.sessions.size;
                    const started = this.callTracker.callsStarted || 0;
                    const memoryUsage = process.memoryUsage().heapUsed / 1024 / 1024;
                    this.sharedState.backoffActive = memoryUsage > 800;
                    logging.log(constants_1.LogVerbosity.INFO, `[API AUDIT] Load: ${started} calls | Active Sessions: ${activeSessions} | Heap: ${memoryUsage.toFixed(2)}MB | Status: ${memoryUsage > 800 ? 'RESOURCE_STRAIN' : 'OPTIMAL'}`);
                }, 60000).unref();
            ...
            _streamHandler(extraInterceptors, stream, headers) {
                // for handling idle timeout
                this.onStreamOpened(stream);
                this.callTracker.addCallStarted(); // Ensure started count works even without full Channelz
